 ...
 * 3. Serial Plotter Diagnostic Logs: Outputs telemetry in a comma-separated format ("Line_Offset,Correction_PWM")
 *    so developers can tune gains visually using the Arduino Serial Plotter.
 * 4. Automatic Calibration Bootstrap: Retains the Day 45 boot-calibration routine, compiling and normalizing
 *    sensor data before launching the active control loop.
 * 
 * STEERING PD CONTROL THEORY:
 * - Line Centroid Offset (Error):
 *     Error (e) = Line_Centroid - Target_Position = Line_Centroid  (Scale: -2.0 to +2.0)
 * - Proportional Gain (Kp): Writes a correction output directly proportional to the distance from center:
 *     P = Kp * e
 * - Derivative Gain (Kd): Measures the rate of change of error (how fast the robot is crossing the line):
 *     D = Kd * ( (e - last_e) / dt )
 * - Total Correction:
 *     Correction = P + D
 ...
 *   If the robot drifts left (error is negative), Correction is negative, slowing down the Left wheel
 *   and speeding up the Right wheel to steer the robot back right.
 * 
 * WIRING:
...
void loop() {
  ...
  if (currentTimeUs - lastPIDTimeUs >= samplePeriodUs) {
    double dt = (double)(currentTimeUs - lastPIDTimeUs) / 1000000.0; // Sample interval in seconds (normally ~0.020s)
    ...
    if (lineDetected) {
      ...
      // Comparing Target (0.0), Actual Offset (-2.0 to +2.0), and scaled PWM correction (divided by 100 to fit on scale)
