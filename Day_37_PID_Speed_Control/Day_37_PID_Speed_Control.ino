 ...
 * 1. Discrete PID Controller: Computes proportional, integral, and derivative terms at a fixed sample rate (10 Hz).
 ...
 * 3. Serial Plotter Integration: Outputs telemetry in a comma-separated format ("Setpoint,Actual_RPM,PWM_Output")
 *    which can be plotted in real-time using the Arduino IDE Serial Plotter.
 ...
 * - Closed-Loop Control: Unlike open-loop systems, closed-loop control continuously measures the system output (RPM)
 *   and adjusts the control output (PWM) to minimize the error:
 *     Error e(t) = Setpoint - Measurement
 * - Control Output Equation:
 *     u(t) = Kp * e(t) + Ki * ∫ e(t) dt + Kd * (de(t) / dt)
 *   - Proportional Gain (Kp): Generates a control signal proportional to the current error. If error is large,
 *     it pushes hard. High Kp speeds up response but causes oscillation.
 *   - Integral Gain (Ki): Sums past error over time (integration). This eliminates steady-state errors
 *     (e.g., if a motor slows down due to friction, the integral sum accumulates, increasing PWM until the target is met).
 *   - Derivative Gain (Kd): Measures the rate of change of error (slope). It acts as a dampener, predicting
 *     approaching setpoints and braking the controller to reduce overshoot.
 * - Anti-Windup: If the motor is stalled or blocked, the error remains high, causing the integral term to grow
 *   infinitely (windup). When the blockage is removed, this massive sum causes the motor to overshoot wildly.
 *   We resolve this by clamping the integral sum.
 * 
 * WIRING:
...
void loop() {
  ...
  if (currentTimeMs - lastPIDTime >= pidSampleTimeMs) {
    double dt = (double)(currentTimeMs - lastPIDTime) / 1000.0; // Time step in seconds (normally ~0.10s)
