 ...
 * 3. The Arduino <Servo.h> library handles generating these exact microsecond pulses in the
 * background using hardware interrupts.
 *
 * WIRING:
 * - Servo Red (VCC)     -> Arduino 5V (For small SG90 servos. Large servos need an external power
 * supply!)
 ...
 * can run directly off the Arduino 5V pin for a simple desktop sweep, larger servos (like
 * MG995/MG996R) will draw enough current to brown-out (reset) the Arduino or damage its voltage
 * regulator. Always use an external 5V-6V power supply (sharing GND with the Arduino) for
 * heavy-load servos!
 */
...
const unsigned long stepDelay =
    15;  // Time to wait between 1-degree steps (15ms is standard for speed)
