 ...
 *    High-power appliances (lightbulbs, AC motors, heaters) run on high voltages (12V, 24V, or
 * 120V/240V AC) and draw amperes of current. Connecting these directly to the Arduino would destroy
 * the chip. A relay provides **galvanic isolation**, separating the low-power control circuit from
 * the high-power load circuit using an air gap.
 ...
 *
 * WARNING:
 * This project is designed for switching low-voltage loads (e.g. a 5V/12V DC motor, battery-powered
 * LED strips).
 * **NEVER work with high-voltage mains electricity (120V/240V AC) unless you are a qualified
 * electrician.** High voltages can cause fatal shocks and fire hazards.
 */
