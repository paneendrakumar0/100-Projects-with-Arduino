 ...
 *   Raw Latitude = 4807.03800. The first two digits are degrees (48°), the rest is minutes
 * (07.038').
 ...
 *   - RX             -> Pin 3 (Arduino Tx, SoftwareSerial - via 4.7k/10k divider to drop 5V
 * to 3.3V)
 */
...
bool parseNMEASentence(String sentence) {
  ...
  if (header.endsWith("RMC")) {
    ...
    if (fields[1].length() >= 6) {
      utcTime = fields[1].substring(0, 2) + ":" + fields[1].substring(2, 4) + ":" +
                fields[1].substring(4, 6);
    }
    if (fields[9].length() == 6) {
      fixDate = fields[9].substring(0, 2) + "-" + fields[9].substring(2, 4) + "-20" +
                fields[9].substring(4, 6);
    }
    return true;
...
void printGPSData() {
  ...
  Serial.print(F(" Date       : "));
  Serial.println(fixDate);
  Serial.print(F(" Fix Status : "));
  Serial.println(fixValid ? F("VALID (Lock Established)") : F("VOID (Searching Satellites)"));
  if (fixValid) {
  ...
  Serial.println(satellitesTracked);
  Serial.println(F("---------------------------------------------------------"));
}
...
void printMenu() {
  ...
  Serial.println(
      F("  * GPGGA (Satellite Status): "
        "$GPGGA,123519.00,1258.9876,N,07735.1234,E,1,08,0.9,105.5,M,-30.0,M,,*41"));
