 ...
 * 2. Impact Phase: Detects a massive acceleration spike ($a_{tot} > 3.0g$) within 500ms of
 * free-fall, indicating collision with the ground.
 * 3. Inactivity/Post-Fall Phase: Monitors the sensor for at least 2.0 seconds after the impact.
 *    If the sensor is lying still (variance $< 0.15g$) and in a tilted orientation (tilt angle $>
 * 45^\circ$), it confirms the subject is down.
...
void printStatus(float aTot, float tilt) {
  ...
  switch (currentState) {
    case STATE_NORMAL:
      ...
      break;
    case STATE_FREE_FALL:
      Serial.print(F("FREE_FALL       "));
      break;
    case STATE_IMPACT_WAIT:
      Serial.print(F("IMPACT_WAIT     "));
      break;
    case STATE_INACTIVITY_CHECK:
      Serial.print(F("INACTIVITY_CHECK"));
      break;
    case STATE_ALARM:
