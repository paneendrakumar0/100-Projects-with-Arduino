 ...
 * 3. Pulse Width Modulation (PWM) Speed Control: Employs analogWrite() on the Enable pin (ENA) to gate
 *    average voltage delivery, regulating motor speeds smoothly.
 * 4. Verbose Telemetry Output: Prints speed percentages, current drive states, and active pin settings
 *    to the Serial Monitor.
 * 
 * H-BRIDGE & DC MOTOR THEORY:
 * - Brushed DC Motors: Rotate when voltage is applied across their two terminals. Reversing the voltage
 *   polarity reverses the magnetic flux direction, reversing shaft rotation.
 * - H-Bridge Circuitry: Direct microcontroller pins cannot output the high current (hundreds of mA to Amps)
 *   needed to drive motors. An H-Bridge uses 4 transistors (BJTs or MOSFETs) in an "H" shape layout.
 *   Selectively turning diagonal pairs of transistors ON directs current through the motor in either direction.
 * - Flyback Protection: DC motors are inductive loads. When current is suddenly cut off, the magnetic field
 *   collapses, creating a high-voltage spike (back electromotive force, or Back-EMF). The L298N module includes
 *   flyback (freewheeling) diodes that shunt these dangerous voltage spikes safely to Ground, preventing
 ...
 * - Power Connections:
 *   - L298N GND      -> Negative (-) terminal of external power source (e.g. 9V battery or DC supply)
 *   - L298N 12V      -> Positive (+) terminal of external power source
 *   - L298N OUT1/OUT2-> Terminals of the brushed DC motor
 * 
 * WARNING: Never power motors directly from the Arduino 5V/3.3V power pins! Motors draw stall currents
 * that can trigger thermal shutdown or permanently damage the Arduino's voltage regulator.
 */
...
void loop() {
  ...
  if (elapsedTime >= stateDuration) {
    ...
    switch (currentMotorState) {
      case STATE_FORWARD_ACCEL:  currentMotorState = STATE_FORWARD_CRUISE; break;
      case STATE_FORWARD_CRUISE: currentMotorState = STATE_FORWARD_DECEL;  break;
      case STATE_FORWARD_DECEL:  currentMotorState = STATE_BRAKING_1;       break;
      case STATE_BRAKING_1:       currentMotorState = STATE_REVERSE_ACCEL;  break;
      case STATE_REVERSE_ACCEL:  currentMotorState = STATE_REVERSE_CRUISE; break;
      case STATE_REVERSE_CRUISE: currentMotorState = STATE_REVERSE_DECEL;  break;
      case STATE_REVERSE_DECEL:  currentMotorState = STATE_BRAKING_2;       break;
      case STATE_BRAKING_2:       currentMotorState = STATE_FORWARD_ACCEL;  break;
    }
...
/**
 * Actively locks the motor armature electronically by grounding both coils (IN1=LOW, IN2=LOW, ENA=HIGH).
 */
void brakeMotor() {
  digitalWrite(IN1_PIN, LOW);
  digitalWrite(IN2_PIN, LOW);
...
void printStateName(MotorState state) {
  switch (state) {
    case STATE_FORWARD_ACCEL:  Serial.print("FORWARD ACCELERATION"); break;
    case STATE_FORWARD_CRUISE: Serial.print("FORWARD CRUISE");       break;
    case STATE_FORWARD_DECEL:  Serial.print("FORWARD DECELERATION"); break;
    case STATE_BRAKING_1:       Serial.print("BRAKE SEQUENCE A");     break;
    case STATE_REVERSE_ACCEL:  Serial.print("REVERSE ACCELERATION"); break;
    case STATE_REVERSE_CRUISE: Serial.print("REVERSE CRUISE");       break;
    case STATE_REVERSE_DECEL:  Serial.print("REVERSE DECELERATION"); break;
    case STATE_BRAKING_2:       Serial.print("BRAKE SEQUENCE B");     break;
  }
}
