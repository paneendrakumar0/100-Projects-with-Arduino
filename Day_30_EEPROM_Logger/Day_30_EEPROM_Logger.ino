 ...
 *    (which destroys the silicon), this sketch distributes writes across all 1024 bytes of the EEPROM,
 *    increasing the maximum lifespan of the memory by a factor of 128 (over 12 million writes).
 ...
 * - Floating Gate Transistors: EEPROM (Electrically Erasable Programmable Read-Only Memory) uses floating-gate
 *   transistors to trap electrons to store bits. Program/erase cycles require high voltages generated internally,
 *   which slowly degrades the oxide insulation layer. The microchip's internal EEPROM is rated for
 *   approximately 100,000 write/erase cycles per byte.
 * - Why Wear Leveling?
 *   If a robot logs data to address 0 every 10 seconds, that byte will wear out and fail in just 11.5 days!
 *   By spreading data updates across the entire memory space sequentially, we ensure the entire chip wears out
 *   together, drastically prolonging active service life.
 * 
 * WIRING:
 * No external components are strictly required as this runs on the Arduino's internal EEPROM.
 * To simulate real-world logging, you can attach an analog sensor (like a potentiometer or LDR) to Pin A0.
 */
...
// --- STRUCT DEFINITIONS ---
struct LogEntry {
  uint32_t logID;       // 4 bytes: Monotonically increasing identifier (used to find chronological order)
...
void scanEEPROM() {
  ...
  if (newestSlot != -1) {
    ...
    activeSlotIndex = -1;
    nextLogID = 1;
    Serial.println("[SYSTEM] Boot scan finished. No valid records found (EEPROM empty or cleared).");
  }
}
...
void readAllLogs() {
  ...
  for (uint32_t targetID = (nextLogID > NUM_SLOTS ? nextLogID - NUM_SLOTS : 1); targetID < nextLogID; targetID++) {
