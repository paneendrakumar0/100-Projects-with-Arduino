 ...
 *
 * DESCRIPTION:
 * This project implements a basic autonomous line-following robot using a two-sensor TCRT5000
 * infrared array. To satisfy mechatronics design standards:
 * 1. Analog Hysteresis Gating: Reads raw analog sensor values and applies software threshold
 * filtering, eliminating the erratic jitter associated with hardware comparator adjustments.
 * 2. Differential Drive Steer Logic: Controls two DC motors via the L298N H-bridge. Steering
 * adjustments are achieved by slowing/stopping one wheel while running the other (differential
 * steering).
 ...
 * - Infrared Reflection: TCRT5000 sensors contain an IR emitting LED and a phototransistor
 * receiver.
 *   - White surfaces reflect high amounts of IR light, turning the phototransistor ON (outputting
 * LOW analog values).
 ...
 *     - Left Sensor < 500 (White) AND Right Sensor < 500 (White) -> Line is centered. Drive
 * Straight.
 ...
 *     - Left Sensor > 500 (Black) AND Right Sensor > 500 (Black) -> T-junction or Stop Line. Stop
 * Robot.
 *
 * WIRING:
