 ...
 *
 * DESCRIPTION:
 * This project interfaces the nRF24L01+ 2.4 GHz radio frequency transceiver module over the SPI
 * bus. To implement a robust, scalable wireless telemetry link:
 * 1. Dual-Role Firmware: Uses a hardware Jumper Pin (Pin 8) to determine role on boot.
 *    If Pin 8 is jumpered to GND, the board operates as a Transmitter. If left open, it acts as a
 * Receiver. This allows the user to compile and upload the exact same sketch to both transceiver
 * nodes.
 ...
 * 3. SPI Control Interface: Configures Chip Enable (CE) and Chip Select Not (CSN) alongside
 * hardware SPI pins.
 ...
 * - nRF24L01+ Transceiver: Runs in the 2.4 GHz ISM band. It splits the band into 125 channels
 * separated by 1 MHz. It uses GFSK modulation and handles packet framing, preamble, CRC error
 * checks, and Auto-ACK internally on the chip hardware, offloading the microcontroller.
 * - SPI Bus Wiring:
 *   The module requires high-speed SPI (MOSI=11, MISO=12, SCK=13).
 *   Additionally, CE (Pin 9) controls RX/TX active modes, and CSN (Pin 10) controls SPI bus
 * selection.
 * - Shared Address Pipeline:
 *   The transmitter and receiver must open a reading/writing pipeline using the exact same 5-byte
 * address (e.g., "00001") and operate on the same frequency channel (default channel 76).
 *
 * WIRING:
 ...
 *   - Pin 8 -> Ground (Connect with a jumper wire to GND to select TRANSMITTER role, leave open for
 * RECEIVER)
 */
