 ...
 *    (SLEEP_MODE_PWR_DOWN), reducing microcontroller current draw from ~15mA down to under 1µA
 * (chip-only).
 * 2. Hardware Interrupt Wake-up: Connects an external tactile button (Pin 2, INT0) that acts as
 *    an active-low hardware interrupt. Pressing the button wakes the processor instantly from
 * sleep.
 ...
 * 4. Inactivity Timer: Implements a non-blocking 5-second timer. If no user interaction is
 * detected, the system automatically enters deep sleep.
 *
 * SLEEP MODES & POWER REDUCTION THEORY:
 * - SLEEP_MODE_PWR_DOWN: In this state, the main crystal oscillator is halted, stopping all
 * internal clocks (CPU clock, I/O clock, flash clock). Only asynchronous interrupts (like low level
 * on INT0) or the Watchdog timer can wake the chip.
 * - ADC Power Draw: The ADC contains active comparator circuitry. If the chip is put to sleep but
 *   the ADC is not disabled (`ADCSRA` bit `ADEN` = 0), the comparator remains powered and drains
 * current.
 * - Board-level Power: Note that standard Arduino Uno boards contain a USB-to-Serial chip
 * (ATmega16U2) and a 5V linear regulator that continue to draw ~10-15mA even when the main
 * microcontroller is in deep sleep. For true micro-ampere operations, a bare chip (like an
 * ATmega328P on a breadboard) or a board with removable LEDs/regulators (like the Arduino Pro Mini)
 * must be used.
 *
 * WIRING:
