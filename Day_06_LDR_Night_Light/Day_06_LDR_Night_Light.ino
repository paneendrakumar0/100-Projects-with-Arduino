 ...
 * 3. Hysteresis Logic:
 *    Instead of using a single threshold (e.g., "if light < 400, turn ON; else, turn OFF"), we use two:
 *    - Turn ON Threshold (Dark): 350
 *    - Turn OFF Threshold (Light): 450
 *    If the reading is between 350 and 450, the LED simply maintains its current state. This 100-point
 *    "dead band" prevents noise from causing rapid transitions.
 * 
 * WIRING:
 * - 5V               -> LDR Pin A
