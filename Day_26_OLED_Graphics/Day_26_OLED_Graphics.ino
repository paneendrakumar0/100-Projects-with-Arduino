 ...
 *    (an organic compound film) glows when given current. This yields absolute black levels (infinite contrast),
 *    ultra-wide viewing angles, and extremely low power consumption.
 * 2. SSD1306 Driver and I2C Framebuffer:
 *    The OLED has a resolution of 128x64 = 8192 pixels. To control them, the SSD1306 chip uses an internal
 *    graphic RAM. Because microcontrollers have limited RAM, the Adafruit library allocates a local
 *    1024-byte framebuffer in the Arduino's SRAM (128x64 bits = 8192 bits = 1024 bytes).
 *    All drawing functions (drawCircle, print, etc.) write to this local SRAM buffer. The `display.display()`
 *    call then transmits the entire 1kB buffer over the I2C bus at high speed, updating the screen.
 * 
 * WIRING:
