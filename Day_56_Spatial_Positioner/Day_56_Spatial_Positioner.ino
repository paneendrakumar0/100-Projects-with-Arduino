 ...
 * - Out-of-bounds math gate: Noise can cause r1^2 - x^2 < 0 (mathematically impossible on clean circles).
 *   We intercept this case to prevent NaN (Not a Number) floating-point errors.
 * 
 * WIRING:
  ...
  if (millis() - lastScanTimeMs >= scanIntervalMs) {
    ...
    if (r1 > 2.0 && r1 < 150.0 && r2 > 2.0 && r2 < 150.0) {
      ...
      if (yRadicand >= 0.0) {
        ...
        Serial.print(F("R1_Dist:"));   Serial.print(r1, 1);
        Serial.print(F(",R2_Dist:"));   Serial.print(r2, 1);
        Serial.print(F(",Coord_X:"));  Serial.print(x, 2);
        Serial.print(F(",Coord_Y:"));  Serial.println(y, 2);
      } 
      else {
