 ...
 * 3. Non-Blocking Frame Rate Controller: Standardizes rendering at a target frame rate (e.g., 30
 * FPS) using millis() timing.
 * 4. Diagnostics Overlay: Displays dynamic coordinates and actual calculated FPS (Frames Per
 * Second) in a dedicated UI header bar, separated from the bounce container.
 *
 * PHYSICS THEORY:
 ...
 *   We reposition the ball exactly at the boundary tangent point to avoid visual clipping or
...
void drawBounceBoundary() {
  ...
  display.drawRect(PLAY_X_MIN, PLAY_Y_MIN - 1, SCREEN_WIDTH, PLAY_Y_MAX - PLAY_Y_MIN + 2,
                   SSD1306_WHITE);
}
