 ...
 *    raw LSB readings into standard acceleration units of gravity (Gs) using the 16384 LSB/g scale factor.
 ...
 * An accelerometer measures proper acceleration (acceleration relative to free fall). When the sensor
 * sits flat and stationary on a desk:
 ...
 *   - VCC        -> 5V (or 3.3V depending on GY-521 regulator version, 5V is standard on GY-521 boards)
 *   - GND        -> GND
 *   - SCL        -> Pin A5 (SCL)
 *   - SDA        -> Pin A4 (SDA)
...
void loop() {
  ...
  if (currentMillis - lastSampleTime >= SAMPLE_INTERVAL_MS) {
    ...
    if (readAccelRaw(rawX, rawY, rawZ)) {
      ...
      Serial.print(F("g\tY: "));       Serial.print(ay, 3);
      Serial.print(F("g\tZ: "));       Serial.print(az, 3);
      Serial.println(F("g"));
    } else {
