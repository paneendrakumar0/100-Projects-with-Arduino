 ...
 * threads:
 * 1. TaskBlink1 (Priority 1): Drives the onboard LED (Pin 13) at a 1 Hz frequency (500ms ON, 500ms OFF).
 * 2. TaskBlink2 (Priority 1): Drives an external LED (Pin 12) at a 0.5 Hz frequency (1000ms ON, 1000ms OFF).
 ...
 * - Preemptive Scheduling: FreeRTOS runs a tick timer (typically 1ms) using Arduino's hardware Timer0.
 *   At each tick, the scheduler reviews the active task queue. If a higher priority task is ready,
 *   the scheduler performs a context switch: it pushes the current CPU registers onto the active task's
 *   stack, switches the stack pointer to the new task, pops its registers, and resumes execution.
 ...
 * 
 * LIBRARY REQUIREMENT:
 * This sketch requires the "FreeRTOS" library by Phillip Stevens (install via Arduino Library Manager).
 * 
 * WIRING:
...
void TaskDiagnostics(void *pvParameters) {
  ...
  for (;;) {
    ...
    Serial.print(F(" Uptime      : ")); Serial.print(uptimeSeconds); Serial.println(F(" seconds"));
