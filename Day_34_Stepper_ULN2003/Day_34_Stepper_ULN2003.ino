 ...
 * 
 * DESCRIPTION:
 * This project interfaces a 28BYJ-48 unipolar stepper motor using a ULN2003 Darlington driver board.
 * Rather than using standard library wrappers, this code writes a custom stepper driver from scratch:
 * 1. Implements a non-blocking, microsecond-level step scheduler using micros() to regulate rotational speed.
 * 2. Uses an 8-state Half-Step sequencing lookup table to achieve high resolution and smooth torque delivery.
 ...
 * 4. Includes automatic phase shutdown (de-energizing coils) when stopped to prevent motor overheating.
 * 
 * STEPPER MOTOR THEORY OF OPERATION:
 * - Unipolar Steppers: Have 4 coils (phases) with common center-taps connected to VCC (5-12V).
 *   Rotation is accomplished by sequentially grounding individual phases (IN1-IN4) to draw magnetic flux.
 * - Darlington Transistor Array (ULN2003): The Arduino cannot sink the current (~300mA) required by the coils.
 *   The ULN2003 contains seven Darlington transistor pairs that act as low-side switches. Grounding an input pin
 *   on the Arduino turns on the corresponding Darlington switch, sinking coil current to Ground.
...
unsigned long stepInterval = 1200;   // Interval between steps in microseconds (sets speed. Min ~1000 for 28BYJ-48)
