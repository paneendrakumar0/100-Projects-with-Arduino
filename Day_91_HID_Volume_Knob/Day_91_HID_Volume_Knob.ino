 ...
 * A rotary encoder contains two internal contact switches (Channel A and Channel B) that are
 * mechanically offset. As the shaft rotates, the switches produce square-wave signals that are 90
...
// For native media keys (Volume Up, Down, Mute) on ATmega32U4, we use the standard HID-Project
// library. If the user hasn't installed HID-Project, we fall back to standard Keyboard shortcuts.
