  ...
  if (Serial.available() > 0) {
    ...
    switch (cmd) {
      ...
      case 'S':
        ...
        Serial.print(F("[INPUT] Target Coordinate set to: X="));
        ...
        if (solveInverseKinematics(targetX, targetY, targetZ, jointTheta1, jointTheta2,
                                   jointTheta3)) {
          writeServos(jointTheta1, jointTheta2, jointTheta3);
          printJointAngles();
        }
...
bool solveInverseKinematics(float x, float y, float z, float& t1, float& t2, float& t3) {
  ...
  if (t1 < 0.0f || t1 > 180.0f || t2 < 0.0f || t2 > 180.0f || t3 < 0.0f || t3 > 180.0f) {
    Serial.println(
        F("[ERROR] Target reachable, but requires joint angles outside servo limits (0-180)!"));
    return false;
  }
...
void runCircleTraceSimulation() {
  ...
  if (solveInverseKinematics(targetX_sim, targetY_sim, targetZ_sim, t1, t2, t3)) {
    ...
    Serial.print(targetX_sim);
    Serial.print(F(",TargetY:"));
    Serial.print(targetY_sim);
    Serial.print(F(",TargetZ:"));
    Serial.print(targetZ_sim);
    Serial.print(F(",BaseAngle:"));
    Serial.print(t1);
...
void printJointAngles() {
  ...
  Serial.print(F(" Base (Servo D9)     : "));
  Serial.print(jointTheta1, 1);
  Serial.println(F(" degrees"));
  Serial.print(F(" Shoulder (Servo D10): "));
  Serial.print(jointTheta2, 1);
  Serial.println(F(" degrees"));
  Serial.print(F(" Elbow (Servo D11)   : "));
  Serial.print(jointTheta3, 1);
  Serial.println(F(" degrees"));
  Serial.println(F("-------------------------------------------------------"));
}
