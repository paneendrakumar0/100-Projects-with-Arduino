...
void runFilterFusionStep() {
  ...
  if (printPlotter) {
    Serial.print(F("TrueAngle:"));
    Serial.print(trueAngle, 2);
    Serial.print(F(",RawAccel:"));
    Serial.print(rawAccel, 2);
    Serial.print(F(",CompFilter:"));
    Serial.print(compAngle, 2);
    Serial.print(F(",Kalman:"));
    Serial.print(filteredAngle, 2);
    Serial.print(F(",EstBias:"));
...
void handleCLICommand(char cmd) {
  ...
  switch (cmd) {
    ...
    case '1':
      ...
      Serial.print(F("[CONFIG] R_measure (Sensor Noise) increased to: "));
      Serial.println(R_measure, 2);
      break;
    case '2':
      R_measure = constrain(R_measure - 0.05f, 0.01f, 2.0f);
      Serial.print(F("[CONFIG] R_measure (Sensor Noise) decreased to: "));
      Serial.println(R_measure, 2);
      break;

    case '3':
      Q_angle = constrain(Q_angle + 0.001f, 0.0001f, 0.1f);
      Serial.print(F("[CONFIG] Q_angle (Process Noise) increased to: "));
      Serial.println(Q_angle, 4);
      break;
    case '4':
      Q_angle = constrain(Q_angle - 0.001f, 0.0001f, 0.1f);
      Serial.print(F("[CONFIG] Q_angle (Process Noise) decreased to: "));
      Serial.println(Q_angle, 4);
      break;
