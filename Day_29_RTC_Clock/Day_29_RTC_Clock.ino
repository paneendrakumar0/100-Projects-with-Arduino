 ...
 *   temperature changes, causing time offsets of minutes per week. The DS3231 incorporates an internal 
 *   32.768 kHz Temperature Compensated Crystal Oscillator (TCXO). An internal temperature sensor
 *   measures temperature and adjusts the load capacitance of the crystal to maintain ±2ppm accuracy
 *   (under 1 minute of drift per year).
 * - Binary Coded Decimal (BCD): The DS3231 stores time data in BCD format. In BCD, each decimal digit
 *   is encoded into a 4-bit nibble. For example, the decimal number 45 is stored in a single byte as
 *   0x45 (which is 0100 0101 in binary) instead of its true mathematical binary representation 0010 1101.
