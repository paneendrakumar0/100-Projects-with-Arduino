 ...
 *    requires 12 pins. To show different numbers on each digit, the pins must be cycled (multiplexed)
 *    at high speed (e.g. Digit 1 ON for 2ms, Digit 2 ON for 2ms, etc.), which requires continuous
 *    CPU cycles.
 * 2. TM1637 Dedicated Driver:
 *    The TM1637 chip handles the multiplexing internally. It has internal RAM to store the digit patterns
 *    and continuously scans the segments in the background.
 * 3. 2-Wire Serial Communication:
 *    The Arduino communicates with the TM1637 using a custom synchronous 2-wire serial protocol (not standard I2C):
