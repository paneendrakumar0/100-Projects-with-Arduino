 ...
 * 1. The code utilizes the standard Keypad library, which handles matrix scanning and key debouncing
 *    in a clean, non-blocking manner.
 ...
 *    to an individual Arduino pin, it would consume 16 pins, leaving no space for displays or motors.
 *    Instead, we place the buttons in a 4x4 grid (Matrix), tying row contacts together and column
 *    contacts together, requiring only 8 pins (4 rows + 4 columns).
 * 
 *        Row 1 Pin ----- (1) --- (2) --- (3) --- (A)
 *                         |       |       |       |
 *        Row 2 Pin ----- (4) --- (5) --- (6) --- (B)
...
void loop() {
  ...
  if (customKey) {
    // Visual indicator: Flash the feedback LED for 100ms (blocking flash is acceptable for simple feedback,
    // but in large loops we use a millis LED timer. Here we keep it simple for immediate response)
