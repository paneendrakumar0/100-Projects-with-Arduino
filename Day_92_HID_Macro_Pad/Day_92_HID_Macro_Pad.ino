 ...
 * tricks the microcontroller into thinking a 4th key is pressed. Placing a signal diode (e.g.,
 * 1N4148) in series with every switch allows current to flow in only one direction, breaking the
 * sneak path.
 *
 * WIRING:
...
bool keyStates[NUM_ROWS][NUM_COLS] = {{false, false, false}, {false, false, false}};
...
const MacroKey macroMap[NUM_ROWS][NUM_COLS] = {
    ...
    {{"Ctrl+S (Save)", actionSave},
     {"Browser Home", actionBrowserHome},
     {"Lock PC (GUI+L)", actionLockScreen}}};
