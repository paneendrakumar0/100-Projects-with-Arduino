 ...
 * 4. Dual-Console Telemetry: Mirrors decoded Bluetooth actions to the PC Serial Monitor for
 * diagnostics.
 *
 * UART & BLUETOOTH HARDWARE THEORY:
 * - UART (Universal Asynchronous Receiver-Transmitter): A serial protocol that transmits data
 * bit-by-bit without a shared clock signal. Synchronization is achieved using Start and Stop bits.
 * Baud rate defines transmission speed (e.g. 9600 bits per second).
 * - RX/TX Voltage Mismatch:
 *   The Arduino Uno runs on 5V logic. The HC-05 runs on 3.3V logic. While the Arduino RX pin can
 * read 3.3V logic safely, the Arduino TX pin outputs 5V. Supplying 5V directly to the HC-05 RX pin
 * can overheat and degrade the chip. A resistor voltage divider (1kΩ and 2kΩ) must be placed on the
 * Arduino TX -> HC-05 RX line to drop the 5V logic signal safely down to 3.3V. Voltage Out = 5V * [
 * 2kΩ / (1kΩ + 2kΩ) ] = 3.33V
 *
 * WIRING:
...
void setup() {
  ...
  Serial.println(
      "[SYSTEM] Send commands: F; (Forward), B; (Backward), L; (Left), R; (Right), S; (Stop)");
}
