 ...
 * 1. Dual-Path Reading: The code monitors the sensor's Analog Output (to measure flame
 * size/distance) and its Digital Output (which acts as a fast comparator trigger).
 * 2. Adaptive Telemetry Rate: When idle, the system logs data once every 500ms. If a flame is
 * detected, it shifts to a high-speed 100ms logging rate to capture emergency event details.
