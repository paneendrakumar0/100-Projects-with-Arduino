 ...
 * - Independent Oscillator: The WDT is driven by an independent internal 128 kHz watchdog
 * oscillator on the ATmega328P. This ensures that even if the main crystal oscillator halts or the
 * CPU registers get corrupted by electromagnetic interference (EMI) from motors, the WDT still
 * counts down.
