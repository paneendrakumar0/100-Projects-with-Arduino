 ...
 * 1. Step/Direction Interface: Simplifies GPIO overhead by utilizing only two signal pins (STEP, DIR)
 *    to control speed and shaft angle.
 ...
 * 3. Microsecond Pulse Gating: Uses micros() timing to trigger step pulses non-blockingly at frequencies
 ...
 * - Bipolar Steppers: Have two internal coils (A and B) with no center-taps. Changing direction requires
 *   reversing current flow inside the entire coil. This requires a dual H-bridge driver.
 * - A4988 Microstepping Driver: Contains a dual H-bridge with adjustable current-limiting (chopping).
 *   Instead of raw coil switching, it regulates current to sinusoidal fractions (microstepping).
 *   This divides a standard 1.8° full step into 1/2, 1/4, 1/8, or 1/16 subdivisions, resulting in:
 ...
 *   Due to rotor and load inertia, a stepper motor cannot instantly start at high speeds; doing so causes
 *   the magnetic field to outpace the rotor, leading to motor stall (slipping poles). We must ramp speed:
