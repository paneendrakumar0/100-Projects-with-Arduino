 ...
 * 1. Direct I2C Register Pipeline: Interfaces the MPU6050 over I2C at a high sample rate (50 Hz /
 * 20ms loops) using raw register transactions via Wire.h.
 * 2. Accelerometer Gravity-Vector Trig: Resolves absolute static tilt angles using trigonometric
 * atan2 mapping.
 * 3. Gyroscope Rate Integration: Non-blockingly integrates angular velocity vectors using dynamic
 * time steps (dt).
 * 4. Complementary Sensor Fusion: Blends the high-pass filtered gyroscope data (trusting short-term
 * changes) and low-pass filtered accelerometer data (trusting long-term gravity orientation) to
 * eliminate both high-frequency vibration noise and low-frequency gyroscopic drift.
 * 5. Serial Plotter Formatting: Outputs telemetry in a comma-separated format
 * ("Roll_Accel,Roll_Gyro,Roll_Fused") specifically designed to demonstrate sensor fusion visually
 ...
 *   Even after calibration, small sensor biases integrate into a linear drift that wanders off to
 * infinity over time.
 * - Accelerometer Noise: Accelerometers measure absolute tilt using the gravity vector. However,
 * linear acceleration (e.g., motor vibration or movement) corrupts the gravity vector, making the
 * raw angle extremely noisy: θ_accel = atan2(Y_accel, Z_accel) * (180 / π)
 ...
 *   This is mathematically equivalent to a low-pass filter on the accelerometer and a high-pass
 * filter on the gyroscope, providing a fast, noise-free, and drift-free angle estimate.
 *
 * WIRING:
...
// Trusts Gyro integration for 98% of the step, and corrects 2% of the way toward Accelerometer
// gravity
...
void loop() {
  ...
  if (currentTimeUs - lastLoopTimeUs >= samplePeriodUs) {
    ...
    if (readRawAccel(&rawAccelX, &rawAccelY, &rawAccelZ) &&
      ...
      // Comparing raw Accelerometer angle (noisy), pure Gyro angle (drifty), and Fused angle
      // (ideal)
