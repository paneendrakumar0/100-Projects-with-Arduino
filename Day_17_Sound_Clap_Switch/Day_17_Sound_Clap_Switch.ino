 ...
 *    Since a physical clap is a decaying sound wave that echoes, the sensor will trigger dozens of
 * times during a single clap. The lockout window ignores these subsequent echoes, preventing
 * double-toggles.
 ...
 *    diaphragm placed close to a rigid backplate, forming a capacitor. Sound waves (air pressure
 * waves) vibrate the diaphragm, changing the capacitance and generating a tiny AC voltage.
 * 2. LM393 Comparator IC: The module contains an operational amplifier acting as a comparator.
 *    - It compares the microphone's analog voltage to a reference threshold set by the onboard
 * potentiometer.
 *    - When the sound is quiet: The output pin (OUT / DO) remains HIGH (5V).
 *    - When a loud sound (like a clap) exceeds the threshold: The output pin instantly pulls LOW
 * (0V).
 ...
 *    on and off rapidly, ending in a random state), we lock out the sensor for 250ms after the
 * first trigger.
 *
 * WIRING:
...
const unsigned long lockoutDuration =
    250;  // Lockout window in milliseconds to ignore echoes (250ms is ideal)
