 ...
 *
 * DESCRIPTION:
 * This project interfaces a TSOP38238 / VS1838B infrared receiver to decode signals from an IR
 * remote. Using the modern IRremote library (v4.x):
 ...
 * 4. Outputs verbose telemetry to the Serial Monitor to allow calibration and discovery of code
 * hexes for any unknown custom remote.
 *
 * INFRARED PROTOCOL THEORY (NEC Protocol):
 * - IR communication uses infrared light pulses modulated at a carrier frequency of 38 kHz to
 * filter out ambient IR light from the sun or lightbulbs.
 ...
 * - A standard frame consists of 32 bits: 8-bit Address, 8-bit inverted Address, 8-bit Command,
 * 8-bit inverted Command. The inverted bytes are used to verify transmission integrity (parity).
 *
 ...
 *
 * LIBRARY REQUIREMENT:
 * This code requires the "IRremote" library by Armin Joachimsmeyer (install v4.x via Library
 * Manager). Do NOT use older 2.x versions as they use outdated and blocking APIs.
 */

// Define the signal receive pin before including the library header to allow internal
// configurations
