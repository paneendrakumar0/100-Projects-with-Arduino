  ...
  if (currentMicros - lastLoopTime >= LOOP_PERIOD_US) {
    ...
    if (readIMUData(rawAx, rawAy, rawAz, rawTemp, rawGx, rawGy, rawGz)) {
      ...
      if (currentMillis - lastPrintTime >= 100) {
        ...
        Serial.print(F("ACCEL ONLY -> Roll: ")); Serial.print(accRoll, 1);
        Serial.print(F("\tPitch: "));           Serial.print(accPitch, 1);
        Serial.print(F(" | GYRO ONLY -> Roll: "));  Serial.print(gyroAngleX, 1);
        Serial.print(F("\tPitch: "));           Serial.print(gyroAngleY, 1);
        Serial.print(F(" | FUSED -> Roll: "));      Serial.print(fusedRoll, 1);
        Serial.print(F("\tPitch: "));           Serial.print(fusedPitch, 1);
        Serial.println();
      }
    }
  }
...
void calibrateGyro() {
  ...
  Serial.print(F("Offsets -> Gx: ")); Serial.print(gyroOffsetX, 1);
  Serial.print(F("\tGy: "));          Serial.print(gyroOffsetY, 1);
  Serial.print(F("\tGz: "));          Serial.println(gyroOffsetZ, 1);
  Serial.println(F("--------------------------------------------------"));
}
 ...
 */
bool readIMUData(int16_t& ax, int16_t& ay, int16_t& az, 
                 int16_t& temp, 
