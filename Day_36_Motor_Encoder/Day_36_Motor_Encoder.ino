 ...
 * 4. Active PWM Sweep: Drives a DC motor via a PWM pin, plotting the relationship between control duty cycles
 *    and physical output speed (measured RPM).
 * 
 * ENCODER & VELOCITY PHYSICS:
 * - Optical Slot Encoders: Contain an infrared LED emitting light toward a photo-transistor receiver,
 *   separated by a slotted disk attached to the motor shaft. As the disk spins, slots block and pass
 *   the light beam, generating a digital pulse train (HIGH/LOW square wave).
